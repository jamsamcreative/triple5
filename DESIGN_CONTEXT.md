...
Light stone-50 section (follows the dark Why band, alternating rhythm), centered uppercase display heading, 4-up white cards (hairline stone border, `--radius-lg`, soft shadow, lift on hover only when the card links out). Card anatomy: **initial avatar** on a red-100 disc with a red letter (a photo-free way to keep the accent present; Google/Yelp avatar hot-linking is unreliable and Yelp's terms are strict), uppercase Barlow name + muted location, **platform badge** as an outlined pill with the platform's own mark **plus its name** (Google / Yelp / Facebook / Trustpilot marks are the only non-brand colours allowed, because they're trademarks, not palette) — sits on the star row, right-aligned, and is the card's outbound link (cards themselves are not links, so there's exactly one link per card). A **platform row** under the heading ("Read our reviews on · Google 5.0★ ↗ · Trustpilot 4.9★ ↗") gives the section's trust signal in one glance and links to each review page; it uses the same outlined-pill language, with a red ↗ as the only accent, **amber stars** using `--color-warning` (same as the hero review badges — one star colour site-wide), body copy 17px. **Filtering is a product rule, not a style one:** default minimum 4★ hides 1–3★; individual reviews can be hidden in Settings regardless of rating. Sample reviews are placeholders — never present them as real.
...
### Project Gallery (`.t5-gallery`, Cornerstone "Triple 5 Project Gallery") — the Projects page
White section, centered eyebrow/heading/subtext, then **filter pills** (outlined stone; the active one fills solid red — the only filled red in the section, so the current filter is the obvious state) and a 3-up card grid. Cards: 16:10 photo (or the blueprint placeholder in rotating charcoal/red/coal tones, same motif as the services grid) with a **red play disc** when a video exists and a mono photo-count chip; below, red uppercase category, uppercase Barlow title, pin + location. Lightbox is charcoal at 92% with round translucent controls that go red on hover; title + mono counter beneath the stage. Content is data-driven (Projects CPT) so the element never holds copy — the owner's categories become the pills automatically.

### Contact (`.t5-contact`, Cornerstone "Triple 5 Contact") — the Contact page
Stone header band (same as service/location pages) then details-left / form-right. The form is literally the hero's lead form (one form, one pipeline, one look site-wide), in a **solid** charcoal card here because the hero's 78% translucent card only works over a photo. Details column: red outline icons for phone/email/location/hours, then **numbered red discs** for "what happens next" — the disc echoes the reasons list; numbers (not checks) because it's a sequence. Map slot reuses the blueprint placeholder. Form column is sticky on desktop so the CTA stays in view while reading. A short FAQ follows to pre-empt "is it free / how fast / do you do Idaho".
